...
## Build in Copilot; host later in Codex

Copilot is the primary local builder using the user's existing subscription. Read BUILD_HANDOFF.md for the portable runtime contract and division of responsibility. Complete all locally feasible implementation, integration, security checks and browser testing before handing back to Codex. Missing Sites tools is not a reason to stop after a scaffold or mock UI. Copilot must not register, publish, obtain Sites credentials, or provision hosted resources. Codex later owns Sites registration, production bindings/secrets, platform identity verification and deployment.

Copilot may edit the local application normally. Sites lifecycle restrictions below govern the Codex hosting phase and agents actually using Sites; they do not prohibit the independent Copilot builder from implementing local source. If using Copilot subagents, assign non-overlapping work and retain one integration owner; respect any active tool/skill restrictions.
...
- In Copilot, preferred root orchestrator: Sol at medium reasoning where supported. Sol owns planning, module contracts, integration and final review; delegate most bounded implementation to Terra, narrow mechanical work to Luna, and exceptional difficult investigations to Astra. Do not have Sol routinely rewrite reviewed Terra work.
- In Codex for the later hosting phase, preferred root orchestrator: gpt-5.6-terra, medium reasoning, standard speed. The app/model selector controls the actual root model; this file cannot switch it. If a mismatch is known, disclose it before substantive implementation and explain how to select Terra. Do not claim a switch occurred without evidence.
...
The user confirms Terra, Sol, Astra and Luna are available in Copilot. Use the explicit model roles above; choose supported reasoning controls and record the actual model selected. Do not assume Copilot request multipliers, pricing, or tool interfaces match Codex. Do not switch providers or buy access. Sol orchestration is intentional; the bounded Sol-review guidance above applies to delegated reviews, not to prohibiting its orchestration role.

The user authorizes bounded subagent delegation under this routing. Delegate only when useful independent work exists; default to at most two active subagents and fewer when coordination costs dominate. Specify model and effort explicitly. For model overrides, use a fresh/minimal context (in Codex, fork_turns="none" or a supported limited history; in Copilot, the supported equivalent), with only relevant requirements, files and acceptance criteria. Do not fork this entire conversation. Ask for concise findings/proposed patches and evidence; avoid repeated broad reviews. Record each delegation's model, purpose and outcome in the milestone handoff.

During the Codex Sites phase, Sites ownership rules apply: the root owns Site checkout edits, Sites tools, registration, credentials, source/version operations and deployment. Subagents provide bounded read-only reviews, fixtures or proposed patches for root integration. They do not edit or initialize the Site or deploy it. Do not reimplement the same solution merely to integrate a reviewed patch.
...
Respect the repository's configured control-clone hook. Make edits/commits in a topic worktree when required; use existing repository scripts if present, otherwise standard `git worktree` commands. Do not disable hooks. Preserve planning documents when scaffolding. Do not push or create a GitHub remote without user direction. Hand off the exact commit and worktree/branch; fast-forward the control checkout when safe and appropriate.
