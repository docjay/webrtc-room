...
In Copilot, choose from the models actually available under the subscription; do not assume Terra/Luna/Sol/Astra names, reasoning controls, prices or Codex tools are available there. Apply equivalent roles: balanced model for implementation, economical model for mechanical work, stronger model for bounded complex reviews. Do not switch providers or buy access to reproduce the Codex model roster. Record the actual selected model when known.
