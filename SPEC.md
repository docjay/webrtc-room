...
- Throughput: sequential A-to-B and B-to-A tests use an adaptive sample target, defaulting to 3 seconds and configurable from 1–10 seconds. Each direction sends at least 8 MiB when the path and deadline permit, then continues until the duration target is reached. A configurable per-direction safety ceiling defaults to 100 MiB (8–256 MiB); total payload is at most twice that ceiling plus protocol overhead. The execution deadline is at least 5 seconds and grows to the configured duration target. A byte ceiling remains authoritative, so a fast path that reaches it before the duration target is explicitly reported as cap-limited rather than as a stable full-duration estimate. Report if a cap, cancellation, visibility change, timeout, or connection change truncated measurement.
