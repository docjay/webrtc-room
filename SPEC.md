...
- TURN input augments configured STUN defaults. Validate schemes, ports, length, and structure; show a sanitized preview. Keep temporary TURN credentials in browser memory only, never in URLs, database records, exports, or browser persistent storage. User-approved update (2026-09-20): display the shared TURN access code as readable text and persist only that code in this site's localStorage, restoring it when the page opens. Saving happens on edits; clearing the field removes the saved value. Leaving/losing access to a room clears temporary credentials and applied configuration, not the saved code. Explain plaintext browser storage and surface storage failures without logging the code. Restoring a code does not bypass room authorization or automatically replace active relay configuration; existing create/join and explicit Apply behavior remains. The code must still stay out of URLs, server diagnostic records and exports. Refresh short-lived Xirsys credentials immediately before Xirsys-backed relay probes so peer wait time cannot leave stale credentials. Each device configures its own credentials.
