...
The user confirms Sol, Terra, Luna and Astra are available. Use Sol as orchestrator, Terra for most implementation, Luna for narrow mechanical tasks, and Astra for justified difficult escalations. Use medium reasoning by default where supported, short assignments and one integration owner; avoid duplicate work. Copilot costs and tool interfaces must not be inferred from Codex. No user-imposed session/token budget applies; do not add allowance checkpoints. Do not change billing or provision paid services.
